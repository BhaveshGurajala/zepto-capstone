...
1. **Ingestion — `app/ingest.py`.** It reads the 8 policy files in `docs/` and makes one chunk per document (`doc_01` … `doc_08`). Each document is short (about 55–89 words) and covers only one policy, so splitting it further would only separate related sentences.
2. **Embedding — `app/ingest.py`.** Embedding turns each chunk into a list of numbers (a vector) using the local model `all-MiniLM-L6-v2`, and stores it in the ChromaDB collection `zepto_policies` using cosine distance. The app builds this automatically at startup if it's missing.
3. **Retrieval — the `retrieve_and_answer` node in `app/graph.py`.** It turns the question into a vector the same way we stored our documents, then asks ChromaDB for the 3 most similar chunks. This always runs for real, even in mock mode.
4. **Generation — `retrieve_and_answer` and `direct_answer` in `app/graph.py`.** In mock mode, it replies "Based on the retrieved context: …" with the first ~200 characters of the best chunk, or gives the fixed "I can only answer…" message for general questions. Every answer is checked by the Pydantic model `AskResponse` (`app/schemas.py`).
...
The keyword list is fixed by the brief (`delivery`, `return`, `refund`, `membership`, `tracking`, `cancel`, `gift card`, `support hours`), so some real policy questions don't match any keyword and get sent to `direct_answer`, even though the answer is in our documents. For example, "How much does Zepto Pass+ cost?" doesn't contain "membership". With `MOCK_LLM=0`, the real LLM classifier would catch this. In mock mode, a fix would be a longer keyword list or a similarity threshold on retrieval, but I kept the list exactly as the brief specified.
